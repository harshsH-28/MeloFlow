...
import { getOrCreateUserTracking } from "@/lib/userTracking";
...
const Home = () => {
  ...
  useEffect(() => {
    const initializeUser = async () => {
      try {
        const userData = await getOrCreateUserTracking();
    ...
    <div className="min-h-screen bg-[#121212] text-[#DCEC7C]">
      ...
      <main className="ml-64 p-8">
        <header className="flex justify-between items-center mb-12">
          <SearchBar className="w-64" onResults={handleSearchResults} />
          <Button
            variant="ghost"
            size="icon"
            className="rounded-full text-[#DCEC7C] hover:bg-[#DCEC7C] hover:text-[#121212]"
          >
                  ...
                  <div className="aspect-square bg-[#1A1A1A] rounded-2xl overflow-hidden relative">
                    {song.thumbnail ? (
                      ...
                      <div className="w-full h-full flex items-center justify-center">
                        <Disc3 className="w-12 h-12 text-[#DCEC7C]" />
                      </div>
                    )}
